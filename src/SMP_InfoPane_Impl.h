...
SMP_InfoPane<smpRGB>::SMP_InfoPane(uint16_t width, uint16_t height, smpRGB* rgbBuffer) : SMP_Pane<smpRGB>::SMP_Pane() {
   this->w = width;
   this->h = height;
   ...
   this->font = (bitmap_font*) fontLookup( font5x7 );
   this->messageGenerator = NULL;
 }


template <typename smpRGB>
bool SMP_InfoPane<smpRGB>::internalSetParent(SM_Layer* parent, LayerType parentType, uint8_t parentDepth) {
...
template <typename smpRGB>
void SMP_InfoPane<smpRGB>::stop(bool clear) {
  this->active = false;
...
template <typename smpRGB>
void SMP_InfoPane<smpRGB>::setMessage(String message, uint32_t currMS) {
  if (this->message != message) {
#if PANE_SERIAL_DEBUG == 1  && PANE_INFOPANE_DEBUG > 0
    Serial.print("SMP_InfoPane::setMessage(): message changed to: "); Serial.println(message);
#endif
...
template <typename smpRGB>
void SMP_InfoPane<smpRGB>::setTextAlign(Align hor, Align vert) {
...
template <typename smpRGB>
bool SMP_InfoPane<smpRGB>::getTextAlignTrans(uint16_t* tX, uint16_t* tY, uint16_t* tW, uint16_t* tH, uint8_t strLen) {
  uint16_t strW = 0;
  uint16_t strH = 0;
...
template <typename smpRGB>
void SMP_InfoPane<smpRGB>::updateContent(uint32_t currMS) {
  uint16_t tx = 0;
  uint16_t ty = 0;
  uint16_t tw = this->w;
  uint16_t th = this->h;
...
template <typename smpRGB>
void SMP_InfoPane<smpRGB>::setFont(fontChoices fontChoice) {
  if (this->contentLayer) {
    this->fontChoice = fontChoice;
    this->font = (bitmap_font*) fontLookup( fontChoice );
    //((SMLayerBackground<smpRGB,0>*)this->contentLayer)->setFont( fontChoice );
  }
};
...
template <typename smpRGB>
void SMP_InfoPane<smpRGB>::setFont(bitmap_font * font) {
  if (this->contentLayer) {
    this->font = font;
